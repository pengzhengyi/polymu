...
type EventSubscription = Map<string, Map<any, EventHandler>>;
...
export class EventNotifier {
  ...
  protected eventSubscription: EventSubscription = new Map();
  ...
  subscribe(subscriber: any, eventName: string, eventHandler: EventHandler) {
   ...
   * @returns {boolean} Whether the unsubscription is successful -- if a subscription previously exists and is removed.
   */

  unsubscribe(subscriber: any, eventName: string): boolean {
    if (this.eventSubscription.has(eventName)) {
  ...
  invoke(eventName: string, ...eventArgs: Array<any>) {
  ...
  disableEventNotification(eventName: string) {
