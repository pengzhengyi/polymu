...
export class SlidingWindow<TElement> implements Iterable<TElement> {
     ...
     */
    constructor(startIndex: number = undefined, endIndex: number = undefined, iterable: Collection<TElement> = undefined, resizeStrategy: ResizeStrategy = ResizeStrategy.Shift) {
    ...
    *[Symbol.iterator](): IterableIterator<TElement> {
    ...
    protected _resizeAndSetWindow(
        ...
        if (this._resizeStrategy === ResizeStrategy.Shift || this._resizeStrategy === ResizeStrategy.ShiftAndShrinkIfNecessary) {
        ...
        if (surplusAmount > 0 && (this._resizeStrategy === ResizeStrategy.Shrink || this._resizeStrategy === ResizeStrategy.ShiftAndShrinkIfNecessary)) {
            /**
             * Assertions:
             * 
    ...
    setWindow(
        ...
        endIndex: number = this._endIndex
    ) {
        ...
        } else { // start index is defined
            if (endIndex === undefined) {
                this._startIndex_ = startIndex;
            } else { // both indices are defined
