...
export class PartialViewScrollHandler<T> extends EventNotifier {
  ...
  get endSentinelIndex(): number {
    return bound(
      Math.floor(this.partialView.windowSize / 4) * 3 - 1,
      0,
      this.partialView.windowSize
    );
  }
  /**
  ...
  private initializeScrollEventListener() {
    const observeTarget =
      this.scrollTarget === document.documentElement ? window : this.scrollTarget;
    observeTarget.addEventListener(
      'scroll',
      debounceWithCooldown((event) => {
        if (
          event.target === this.scrollTarget ||
          (document === event.target && document.documentElement === this.scrollTarget)
        ) {
          // only handle scroll event happening on observed scroll container
          const startIndex = this.getElementIndexFromScrollAmount();
          if (
            startIndex < this.partialView.partialViewStartIndex ||
            this.partialView.partialViewEndIndex < startIndex
          ) {
            // view out of sync
  ...
  private sentinelReachedHandler(entries: Array<IntersectionObserverEntry>) {
    ...
    entries.forEach((entry) => {
      const desiredDirection: Direction =
        this.startSentinelElement === entry.target ? Direction.Up : Direction.Down;
      if (
