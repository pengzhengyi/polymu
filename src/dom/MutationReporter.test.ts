...
describe('Attribute Change', () => {
  ...
  test('Attribute change and catch event', (done) => {
    ...
    reporter.observe(
      element,
      MutationReporter.createMutationObserverInit(true, false, true, false, ['class'])
    );
  ...
  test('Append a text node', (done) => {
    const reporter = new MutationReporter((correctedMutations, observer, mutations, _reporter) => {
      ...
      const characterMutation =
        correctedMutations[0].type === 'characterData'
          ? correctedMutations[0]
          : correctedMutations[1];
...
describe('observing and unobserving', () => {
  ...
  test('re-observe', (done) => {
    ...
    reporter.reconnectToExecute(() =>
      target.replaceChild(document.createTextNode('ignored'), target.firstChild)
    );
